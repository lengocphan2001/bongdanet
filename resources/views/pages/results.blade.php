...
@section('title', 'Kết quả bóng đá - Bongdanet')
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('results', ['type' => 'live']) }}" 
                           class="px-4 py-2 text-sm font-medium text-white {{ ($isLive ?? false) ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-600 hover:bg-gray-700' }} rounded transition-colors">
                            Kết quả trực truyền
                        </a>
                        @foreach ($dateOptions ?? [] as $option)
                            ...
                            <a href="{{ route('results', ['date' => $option['value']]) }}" 
                               class="px-4 py-2 text-sm font-medium text-white {{ $isActive ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-600 hover:bg-gray-700' }} rounded transition-colors">
                                {{ $option['label'] }}
                            </a>
                        @endforeach
                    </div>
