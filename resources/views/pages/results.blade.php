                ...
                <div class="bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 rounded-xl shadow-2xl border border-slate-700/50 p-4 sm:p-6 md:p-8 overflow-hidden backdrop-blur-sm">
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-0 uppercase break-words tracking-tight">
                            <span class="bg-gradient-to-r from-white via-gray-100 to-gray-300 bg-clip-text text-transparent">Kết Quả Bóng Đá</span>
                        </h1>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-slate-800/80 to-slate-900/80 rounded-lg border border-slate-700/50 p-2.5 mb-4 backdrop-blur-sm">
                        <div class="flex items-center gap-1.5 overflow-x-auto scrollbar-hide pb-1 -mx-1 px-1">
                            <a href="{{ route('results', ['type' => 'live']) }}" 
                               class="px-2.5 py-1.5 text-xs font-semibold rounded-md whitespace-nowrap flex-shrink-0 transition-all duration-200 {{ ($isLive ?? false) ? 'text-white bg-gradient-to-r from-emerald-600 to-green-700 shadow-md shadow-emerald-500/20' : 'text-gray-300 bg-slate-700/50 hover:bg-slate-700 hover:text-white' }}">
                                🔴 Trực tiếp
                            </a>
                            @foreach ($dateOptions ?? [] as $option)
                                ...
                                <a href="{{ route('results', ['date' => $option['value']]) }}" 
                                   class="px-2.5 py-1.5 text-xs font-semibold rounded-md whitespace-nowrap flex-shrink-0 transition-all duration-200 {{ $isActive ? 'text-white bg-gradient-to-r from-red-600 to-red-700 shadow-md shadow-red-500/20' : 'text-gray-300 bg-slate-700/50 hover:bg-slate-700 hover:text-white' }}">
                                    {{ $option['label'] }}
                                </a>
                            @endforeach
                        </div>
