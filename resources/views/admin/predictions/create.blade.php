...
<div class="bg-white rounded-lg shadow-md p-3 sm:p-6">
    ...
    <form method="POST" action="{{ route('admin.predictions.store') }}" id="prediction-form" enctype="multipart/form-data" onsubmit="return submitForm()">
        ...
        <div class="mt-6">
            ...
            <textarea id="content" 
                      name="content" 
                      class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#1a5f2f] focus:border-[#1a5f2f]">{{ old('content') }}</textarea>
            @error('content')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mt-6">
            ...
            <textarea id="analysis" 
                      name="analysis" 
                      class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#1a5f2f] focus:border-[#1a5f2f]">{{ old('analysis') }}</textarea>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    matchSelect.addEventListener('change', function() {
        ...
        if (selectedOption.value) {
            ...
            document.getElementById('match_api_id').value = selectedOption.value;
            document.getElementById('league_id').value = selectedOption.dataset.leagueId || '';
            document.getElementById('home_team').value = selectedOption.dataset.homeTeam || '';
            document.getElementById('away_team').value = selectedOption.dataset.awayTeam || '';
            document.getElementById('league_name').value = selectedOption.dataset.leagueName || '';
            ...
            if (matchTime) {
                ...
                document.getElementById('match_time').value = formatted;
            }
        } else {
            // Clear fields
            document.getElementById('match_api_id').value = '';
            ...
            document.getElementById('match_time').value = '';
        }
    });
});
</script>
